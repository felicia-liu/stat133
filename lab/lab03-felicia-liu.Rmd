---
title: "Lab 3: Basic Data Objects"
...
Last week we looked at a couple of approaches to manipulate tables and create 
...
In this lab, you are going to work with the same dataset from last week: the 
data `starwars` from `"dplyr"`. Because you should have `"dplyr"` already 
installed, the only thing you need to do---in order to use the functions---in 
this package is to load it. This time however, we will focus on columns `name`, 
`height`, `mass`, `sex`, and `homeworld`, and we will also remove rows that 
...
As we mention, data frames are made of vectors. Each column of a data frame 
corresponds to an R vector. So let's "break down" the table `dat` into five 
vectors:
...
```{r}
odd_height <- height[c(TRUE, FALSE)]
odd_height
```
...
```{r}
mult5_sex <- sex[c(FALSE, FALSE, FALSE, FALSE, TRUE)]
mult5_sex
```
...
```{r}
mult10_mass <- mass[c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE,
...
```{r}
# your code
naboo_names <- name[homeworld == 'Naboo']
naboo_names
```
...
```{r}
# your code
naboo_corellia_names <- name[homeworld == 'Naboo' | homeworld == 'Corellia']
naboo_corellia_names
```
...
```{r}
# your code
largest_mass_name <- name[mass == max(mass)]
largest_mass_name
```
...
```{r}
# your code
largest_height_females <- max(height[sex == 'female'])
largest_height_females
```
...
```{r}
# your code
name_height_med <- name[height == median(height)]
name_height_med
```
...
```{r}
# your code
name_180height_120mass <- name[height <= 180 & mass >= 120]
name_180height_120mass
```
...
Practice manipulating `sex_fac` to get:

- sex of individuals from homeworld Naboo

```{r}
# your code
sex_from_naboo <- sex_fac[homeworld == 'Naboo']
sex_from_naboo
```
...
```{r}
# your code
sex_mass_over80 <- sex_fac[mass > 80]
sex_mass_over80
```
...
```{r}
# your code
sex_height_over180 <- sex_fac[height > 180]
table(sex_height_over180)
```
...
```{r}
# your code
males_for_homeworlds <- factor(homeworld)[sex_fac == 'male']
table(males_for_homeworlds)
```
...
```{r}
# your code
MH <- cbind(height[1:10], mass[1:10])
MH
```
...
```{r}
# your code
females <- matrix(c(mass[sex == 'female'], height[sex == 'female']),
                  nrow = length(name[sex == 'female']), ncol = 2)
females
```
...
```{r}
# your code
MHW <- matrix(c(mass, height, homeworld),
                  nrow = length(name), ncol = 3)
typeof(MHW)
```
